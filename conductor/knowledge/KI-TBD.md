...
| 4 | No Room migration harness | [CORE_RULES §13](../rules/CORE_RULES.md) | `core/data/` | ⏸️ Blocked | `BipSaleDatabase` sets `exportSchema = false`, so no schema JSON exists to migrate against — flip it to `true` and commit the v1 schema first. Required before the next schema change ships; this DB holds sales records |
...
**#1, #2, #3, #9, #10** are unblocked. Suggested order: #2 (money path) → #3 (known failure paths) → #9 (stops new code shipping unchecked) → #10 (cheap tidy-up). #4 needs `exportSchema = true` committed first.
