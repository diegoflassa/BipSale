...
PLANNING_RULES §25 makes every deferred task name one recommended model from a fixed pool of seven, and a
...
The routing string of each model - its configuration included - is in PLANNING_RULES §25.6 and is not repeated
here.

### 2. The recommended model

1. **Tier the task by its hardest step.** A task with an easy phase and a hard phase is two tasks
   (PLANNING_RULES §25.3), never an average.
2. **Tighten before you tier.** When the risk is underspecification, write the missing files, contract,
   invariants and acceptance checks into the task body - that can move a task from T1 to T2 (§25.5). When the
...
- **Another provider** (§25.4) - never negotiable.
...
**An open money decision (T1).** Where a sale-level or per-line discount is rounded is not yet
decided ([KI-07](KI-07-SALES-CART-AND-DISCOUNTS.md), the `money` skill).

- `Recommended: Claude 5.0 Opus · Think ON · Effort Extra High` - a wrong rounding point costs money at the till
  and passes every test.
- `Fallback: Grok 4.6 · Effort High` - the other T1 model, from another provider.
- `Fallback notes:` read KI-07 and the `money` skill before editing, and state in the task the rounding point the
  change keeps.

**A migration with its mapping written (T2).** A Room schema change whose column mapping is quoted in the task
([CORE_RULES.md](../rules/CORE_RULES.md) §13).

- `Recommended: Claude 5.0 Sonnet · Think ON · Effort Extra High` - the mapping is decided; the risk is a slip.
- `Fallback: Gemini 3.2 Pro High` - same tier, another provider.
- `Fallback notes:` a version bump, a real `Migration`, an exported schema and a passing migration test in the
  same turn; never `fallbackToDestructiveMigration()` - this database holds sales records.
...
- [ ] Every routing names a pool model with its PLANNING_RULES §25.6 routing string, character for character.
