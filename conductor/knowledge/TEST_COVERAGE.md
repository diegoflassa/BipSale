...
> **Every module carries a JVM suite.** 197 JVM tests and 95 instrumented tests: 13 data-layer, 19 QR-rendering, and 63 Compose UI across `:feature:history` (13), `:feature:sales` (19) and `:feature:products` (31). The gap that remains is the `:app` screens.
...
| `:core:utils` | 1 | 0 | `ExcelExporterTest` (11) pins the header, one row per sale item, the date/quantity/line-discount columns, that values stay under the header they belong to, and that the sale total is written once per sale so summing the column cannot double count. |
...
| Command | Purpose |
|---|---|
| `./gradlew test` | All JVM unit suites (197 tests) |
