...
| `[BipSale][Product][QR_EXPORT]` | `feature/products/print/QrLabelPrintAdapter.kt`, `feature/products/print/QrLabelPrinter.kt`, `feature/products/ProductViewModel.kt`, `core/qrcode/QrGenerator.kt` | QR label sheet printing — logs the resolved media size, grid shape, labels-per-page and page count at layout time, the configured column count and both label type sizes at the moment the dialog opens, the label/page totals actually written, and QR encode failures. Warns when a print is requested with nothing to print. | `Timber.d` (progress), `Timber.w` (empty request), `Timber.e` (encode / write failure) |
...
| `[BipSale][Settings]` | `core/data/settings/SettingsRepositoryImpl.kt`, `app/ui/settings/SettingsViewModel.kt` | The configured default PIX discount, customer-info toggle, QR label column count and the two label type sizes — logs a load, a save naming each saved value, and the read failure that fell back to defaults. PIX key, merchant name and city are no longer user-configurable (they live in `PixDefaults`), so the filter no longer monitors them. | `Timber.d` (load/save), `Timber.i` (saved), `Timber.w` (rejected input), `Timber.e` (failure) |
| `[BipSale][Import]` | `core/data/product/ProductImportRepositoryImpl.kt`, `feature/products/ProductViewModel.kt` | Product spreadsheet template and import — logs the destination the template was written to, whether the operator confirmed or cancelled at the overwrite warning, the accepted and rejected row counts, and one line per rejected row naming its one-based row number, its code and why it was left out. | `Timber.d` (progress), `Timber.i` (written/imported), `Timber.w` (rejected row), `Timber.e` (failure) |
