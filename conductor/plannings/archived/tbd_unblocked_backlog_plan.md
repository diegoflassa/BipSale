...
| #7 | Checkout flow has no trace logging | T3 — pulled in by `CORE_RULES §8.2`, since T4–T6 rewrite the money path |
...
`[BipSale][Sale][CHECKOUT]` breadcrumbs across `SalesViewModel` + `FinalizeSaleUseCase`: item added/removed, discount applied, finalize entered, persisted, failed. Redaction per `CORE_RULES §8.3` — never the CPF or the customer name, always the counts and the total. Catalogue the filter in KI-04 in the same turn.
