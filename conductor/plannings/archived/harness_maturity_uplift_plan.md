# Harness maturity uplift — BipSale

**Plan name:** Harness Uplift — from L2 Guided toward L3 Sensing
**Mode:** FEATURE — no root-cause phase.
**Status:** Complete 2026-09-13. T1–T5, T7 done. T9 skipped (optional — D4 = No: `planning` skill already carries the checklist). Revisited on the owner's request the same day: still no subagent (last log entry). T10 done. No T6 or T8 in this plan.
(execution log below); every other task is still plan only, and nothing has been committed.
**Blocking questions:** none. Two owner decisions (D1, D4) each gate one task; every other task is executable.
**Source:** `harness-score` v1.6.5, run 2026-09-13 against this repository right after the Antigravity AI-surface
restructure — **L2 · Guided, 56/108 (52%)**, detected Antigravity + Claude Code.
**Governed by:** [PLANNING_RULES.md](../../rules/PLANNING_RULES.md) §7 and §25, the
...
**Sibling plans (CORE_RULES §15):** Slotify and Comiqueta carry the same shared tasks (T1, T2, T3, T5, T7) in their own
...
quality. Only T1, T2, T5, T9 and T10 meet that bar. T3, T4 and T7 stay on 4.6.

## Task index

| Task | What | Estimated score effect (this scanner) | Routing A — preferred | Routing B — fallback | Depends on |
|---|---|---|---|---|---|
| T1 | ✅ Rename `.agent/` to `.agents/` — done 2026-09-13 | Skills 3 → 12/17 (+9) | Grok 4.20 Reasoning · OFF · Medium | Grok 4.20 Reasoning · OFF · Medium | D1 |
| T2 | ✅ `.gitignore` covers `.env` files — done 2026-09-13 | Hygiene +3 | Grok 4.20 Reasoning · OFF · Low | Grok 4.20 Reasoning · OFF · Low | — |
| T3 | ✅ SessionStart + gate/feedback — Claude Code live; Antigravity `.agents/hooks.json` shipped `"enabled": false` 2026-09-13 | Hooks 6 → 14/14 (+8) | Grok 4.6 · ON · Medium | Grok 4.6 · ON · Medium | — |
| T4 | ✅ CI runs the tests, detekt, ktlint and Android Lint — done 2026-09-13 | CI +7 | Grok 4.6 · High | Grok 4.6 · High | — |
...
There is no T6 or T8 here: detekt and ktlint are already configured, and `LICENSE` already exists. Task numbers match the
sibling plans on purpose, so a shared task has one number in all three.

**Estimate with T1–T5:** about 86/108, **L2 · Guided**. L3 needs Sensors ≥ 60% and CI ≥ 50%. T4 clears the CI gate, but this
scanner cannot credit the existing tests, detekt or ktlint — that cap belongs to the scanner, not to the harness (T7).

## What the report says, and what is real

| Check | Result | Real gap? | Decision |
|---|---|---|---|
| SKL-01, SKL-02, SKL-04 | fail | **No.** 17 project skills and 11 Android skills exist at `.agent/skills/`; the scanner reads skills only from `.agents/skills/`, `.claude/skills/` and `.cursor/skills/` | T1, gated on D1 |
| AGT-01, AGT-02 | fail | Partly. No subagent exists, and Antigravity has no subagent format | T9, optional |
| HKS-01, HKS-02 | pass | `.claude/settings.json` registers a `SessionStart` hook | — |
| HKS-03, HKS-04 | fail | **Yes.** No gate hook and no feedback hook, so CORE_RULES §2 is enforced by prose alone | T3 |
| HKS-05 | fail | **A false positive with a real lesson.** The `SessionStart` command is inline PowerShell, which the scanner parses as a missing script path — but an inline one-liner is also the hardest hook to review or test | T3 |
| SNS-01 | fail | **No.** 287 JVM tests run through `./gradlew test`; the scanner never recognises a Gradle test setup | T7 |
| SNS-02, SNS-04 | fail | **No.** detekt 1.23.8 (`config/detekt/detekt.yml`) and ktlint 14.2.0 are configured; the scanner does not know either | T7 |
| CI-02, CI-03 | fail | **Yes.** The only workflow is `gradle-wrapper-validation.yml` — the tests, detekt and ktlint never run in CI | T4 |
...
## T3 — Hooks as committed scripts: SessionStart, gate and feedback
...
**Files:**
- `tools/hooks/session_start_index.py` — the existing inline `SessionStart` command moved into a file, unchanged in behaviour:
  it reads `conductor/knowledge/INDEX.md` and emits it as `additionalContext`. `.claude/settings.json` then runs
  `python "$CLAUDE_PROJECT_DIR/tools/hooks/session_start_index.py"`. Python rather than a `.ps1` file: PowerShell's
  execution policy can refuse a script file where it allowed the inline command, which would drop the hook silently.
...
- `tools/hooks/test_hooks.py` — `unittest` cases fed with recorded payloads for both Python scripts.
- `.claude/settings.json` — beside `SessionStart`: `PreToolUse` matcher `Bash` runs `python tools/hooks/guard_git.py`;
  `PostToolUse` matcher `Edit|Write` runs `python tools/hooks/check_agent_docs.py`.
- Antigravity counterpart — `.agents/hooks.json` (or `.agent/hooks.json` without T1) with the two Python scripts: `PreToolUse`
...
**Verify:** a new Claude Code session still receives the knowledge index; `git status` runs untouched and `git commit` prompts;
editing a skill to an unquoted `description: a: b` prints a finding; `python -m unittest tools/hooks/test_hooks.py` passes;
HKS-01 … HKS-05 pass.

## T4 — CI runs the sensors

**Routing:** A — Grok 4.6 · Think ON · Effort High — a clean checkout and which Gradle tasks belong in CI
are judgement; Opus-class. B — Grok 4.6 · Think ON · Effort High.
**Independently applicable:** yes.

1. **Establish what a clean checkout needs before writing YAML:** whether `google-services.json` (Firebase App Distribution)
   or any other gitignored file is required by the `test`, `detekt`, `ktlintCheck` and `lint` tasks, and which module needs it.
2. Add `.github/workflows/ci.yml` on push and pull request: checkout; the JDK the build declares;
   `gradle/actions/setup-gradle` for caching; any required file decoded from a repository secret — never a real secret
   committed ([SECURITY_RULES.md](../../rules/SECURITY_RULES.md) §24); then `./gradlew test`, `./gradlew detekt ktlintCheck` and
   `./gradlew lint`.
3. Keep `gradle-wrapper-validation.yml` unchanged.

**Verify:** green on a branch; red on a deliberately failing test and on a planted lint error; CI-02 and CI-03 pass.
...
1. Add `.pre-commit-config.yaml` with `repo: local` hooks only: `check_agent_docs.py` over staged `*.md`, and a staged-file
   check for the credential shapes SECURITY_RULES §24 names. No Gradle task — a Gradle start is slower than a commit should be,
   and T4 runs the sensors in CI.
...
`detekt` or `ktlintCheck`; HKS-05 parses an inline command as a script path.
...
- **(b) Draft an upstream issue or PR** for harness-score adding Gradle detection: a `test`/`check` task or `src/test` sources
  ...
  `detekt|ktlint` in CI-03's pattern; inline commands recognised in HKS-05. Filing it is the owner's call.
...
- [ ] A new session still receives the knowledge index (T3).
- [ ] CI green on a branch and red on a planted failure (T4).
...
- D1 and D4 gate T1 and T9. Nothing else is blocked.
...
- **T3** — `tools/hooks/guard_git.py` (PreToolUse — asks before git writes and recursive forced deletes, and before Gradle builds), `tools/hooks/check_agent_docs.py` (PostToolUse — reports frontmatter and link problems in an edited AI document), `tools/hooks/session_start_index.py` (the SessionStart hook, same output as the inline command it replaced), and `tools/hooks/test_hooks.py`; all tests pass (`python -m unittest discover -s tools/hooks -p test_hooks.py`). Registered in `.claude/settings.json`. **Not done:** the Antigravity `hooks.json` — its tool names are unverified, and a matcher naming the wrong
  tool fails open.

| harness-score v1.6.5 | Before | After |
|---|---|---|
| Maturity | L2 · Guided | L2 · Guided |
| Score | 56/108 | 76/108 (+20) |
| Context & Guides | 20/20 | 20/20 |
| Skills & Commands | 3/17 | 12/17 |
| Hooks & Guardrails | 6/14 | 14/14 |
| Sensors & Feedback | 6/20 | 6/20 |
| CI Feedback | 4/14 | 4/14 |
| Hygiene & Safety | 17/23 | 20/23 |

**Still failing:** AGT-01, AGT-02, CI-02, CI-03, CI-04, HYG-08, SNS-01, SNS-02, SNS-04. **Next level per the scanner:** L3 — sensors ≥ 60%; ci ≥ 50%.
...
- **T4** — `.github/workflows/ci.yml` on push and pull_request: JDK 21, Gradle cache, Android SDK, `google-services.json` decoded from `GOOGLE_SERVICES_JSON_BASE64`, then `./gradlew test`, `./gradlew detekt ktlintCheck`, `./gradlew lint`. `gradle-wrapper-validation.yml` unchanged.
...
- **T10:** All tasks complete across three projects. Plans archived. Build validation green.

### 2026-09-13 — T9 revisited on the owner's request (Claude 5.0 Opus · Think ON · Effort Extra High)

- **Still no subagent, for a stronger reason than the checklist.** The paths where a slip costs money or data are already pinned by tests - the whole money path, the migration harness and a Compose suite on every screen, 287 JVM and 125 instrumented tests (`TEST_COVERAGE.md`) - and detekt and ktlint run in CI. What stays manual (logging, strings, KI sync, changelog) is loaded as skills at the moment it binds, and does not justify a delegate.
