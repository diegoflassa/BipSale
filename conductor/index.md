...
- **[ARCHITECTURE.md](rules/ARCHITECTURE.md)** — architecture SOT (module graph, layers, MVI, Hilt, Room/Retrofit, build)
...
- **[AGENT_IO_RULES.md](rules/AGENT_IO_RULES.md)** — agent execution I/O (§21–§23: text encoding and shell output, chunked writing of long artefacts, full token-economy spec)
- **[SECURITY_RULES.md](rules/SECURITY_RULES.md)** — repository safety (§24: what never gets committed, placeholders, what to do when a secret is already in git)
...
| [money](../.agents/skills/money/SKILL.md) | Monetary values - minor units, rounding once | Code carrying a price, total, discount or payment |
...
**Stack:** Hilt · Room (SSOT) · Retrofit · Nav 3 · Compose Material 3 · CameraX · Timber · Firebase App Distribution. Convention plugins in `build-logic/`; config SSOT in `build-logic/Configuracoes.kt`; versioning via `version.properties` (`0.0.2-alpha-build_N`). Static analysis: detekt (all modules via `detekt-convention` plugin). Coverage: Kover.
...
| Analysis | `./gradlew detekt` — runs on all modules via `detekt-convention` plugin |
