...
Add Gradle detection so a Kotlin/Android Gradle project is scored for tests, lint and formatters it actually has. This repository already runs detekt and ktlint; the scanner still does not credit them.
...
| HKS-05 | a script path | an inline command is parsed as a script path and fails the check |

## Out of this draft

Optional `.harness-score.json` exclusions are not applied. HYG-03, HYG-04 and HYG-06 cannot be excluded.
