...
`Timber.i` / `Timber.w` / `Timber.e` are intentional production signal and are **never** touched by `/remove-filter`, regardless of filter.

### 8.5 Renaming or removing a filter

Filter names are public string contracts. Any rename or removal updates, **in the same turn**: the KI-04 row, this section if it names the filter, [`workflows/remove-filter.md`](../workflows/remove-filter.md) if it names the filter, every production `Timber.*` call site, and every test assertion on the tag. One-turn change or none.
