...
### 25.4 Two routings per task: preferred and fallback (MANDATORY)

**Every deferred task carries two routings.** §25.1 still holds — the choice is made per task —
...
Both appear in both places §25.2 requires. In the table the routings alone suffice; at the head of
...
- **Applies to both options of §25.4, independently**, and **is revisited when the task changes**:
