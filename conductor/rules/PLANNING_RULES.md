...
6. **Do NOT start implementing** during the META_PLANNING synthesis turn unless the user explicitly asks. Synthesis = planning only.
  7. **One file.** A synthesis produces **one** plan file (§7). Do not write a sibling edition for
    another model or a file per proposal. Route each task inside it per §25: **one recommended model**
    from the pool with its fixed configuration (§25.6), and a **fallback from a different provider only
    when it produces code of the same quality** (§25.4). **The plan is written by `Claude 5.0 Opus · Think ON · Effort Extra High`.**
...
## 25. Model Assignment for Deferred Tasks (GLOBAL - MANDATORY)
...
### 25.1 One model per task, decided per task (MANDATORY)
...
### 25.2 Annotation format (MANDATORY)

**A routing is one model from the pool (§25.6), written with its configuration exactly as the pool table
...
|---|---|---|
| Recommended | `Recommended: <routing string from §25.6>` | always |
| Fallback | `Fallback: <routing string from §25.6>`, or `Fallback: none — <why>` | always — `none` is an answer |
...
### 25.3 Correctness first (MANDATORY)
...
### 25.4 A recommended model, and a fallback only when it matches (MANDATORY)

**Every deferred task names one recommended model and, only when one exists, a fallback from a different
provider that produces code of the same quality.** §25.1 still holds — both choices are made per task.

| Routing | Meaning |
|---|---|
| **Recommended** | the model most likely to execute this task correctly (§25.3) |
...
Both routings appear in both places §25.2 requires. In the table the routings alone suffice; at the head of
...
- **Fallback notes are written before the fallback is recorded.** A fallback that matches only with extra
  guidance matches only once that guidance is in the task body (§25.5) — a fallback justified by notes nobody
...
- **The decision guide is [KI-10](../knowledge/KI-10-MODEL-SELECTION.md)** — the tier of every pool model,
...
### 25.5 Prefer the cheapest model that still guarantees the outcome (MANDATORY)

§25.3 says correctness decides, and it still does. This is about what happens *after* two
...
- **The bound, and it is absolute: never trade quality for tier.** If the risk is *judgement* rather
  ...
  task stays at the tier §25.3 chose.
...
- **Applies to the recommended model and the fallback of §25.4, independently**, and **is revisited when the task changes**:
  if scope grows or the contract turns out ambiguous, the tier goes back up before execution, not
  after a failed attempt.

### 25.6 One file, one pool (MANDATORY)
...
provider, an effort chosen per task — are re-routed under §25.2–§25.4 **when their task is next planned,
