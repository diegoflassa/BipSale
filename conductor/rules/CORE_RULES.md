...
| **8** | [Logging — Timber Only](LOGGING_RULES.md#8-logging--timber-only) | [LOGGING_RULES](LOGGING_RULES.md) |
|     **8.1** |     [Adding a log](LOGGING_RULES.md#81-adding-a-log-mandatory--read-before-writing-any-timber-call) *(M)* | [LOGGING_RULES](LOGGING_RULES.md) |
...
| **20** | [Monetary Values](#20-monetary-values-global---mandatory) *(M)* | — |
| **21** | [Text Encoding and Shell Output](AGENT_IO_RULES.md#21-text-encoding-and-shell-output-global---mandatory) *(M)* | [AGENT_IO_RULES](AGENT_IO_RULES.md) |
| **22** | [Chunked Writing of Long Artefacts](AGENT_IO_RULES.md#22-chunked-writing-of-long-artefacts-global---mandatory) *(M)* | [AGENT_IO_RULES](AGENT_IO_RULES.md) |
| **23** | [Token Economy Discipline](AGENT_IO_RULES.md#23-token-economy-discipline-global---mandatory) *(M)* | [AGENT_IO_RULES](AGENT_IO_RULES.md) |
| **24** | [Credentials Never Enter the Repository](SECURITY_RULES.md#24-credentials-never-enter-the-repository-global---mandatory) *(M)* | [SECURITY_RULES](SECURITY_RULES.md) |
| **25** | [Model Assignment for Deferred Tasks](PLANNING_RULES.md#25-model-assignment-for-deferred-tasks-global---mandatory) *(M)* | [PLANNING_RULES](PLANNING_RULES.md) |
|     **25.6** |     [One file, one pool](PLANNING_RULES.md#256-one-file-one-pool-mandatory) *(M)* | [PLANNING_RULES](PLANNING_RULES.md) |

| **26** | [Root-Cause Bug Fixes](#26-root-cause-bug-fixes-global--mandatory) *(M)* | — |
...
The full spec — when a re-read is correct, batched documentation sync, what the compiler owns, and the habits that keep the session's cached prefix intact — is [AGENT_IO_RULES.md](AGENT_IO_RULES.md) §23. The paragraph above is the operative summary; if it is enough to act on, do not open that section.
...
- The existing interfaces are reviewed against this rule under [KI-TBD.md](../knowledge/KI-TBD.md) #15. Until that
...
## 20. Monetary Values (GLOBAL - MANDATORY)

Money is never a `Float` or a `Double`, and never a formatted string once it leaves the UI layer.

- **Store and transport integer minor units** (centavos). `total = 12990` is R$ 129,90. An integer has no
  representation error, compares exactly, and sums without drift.
- **Format only at the edge.** Currency symbols, thousands separators and the decimal comma are produced in
  the UI layer from the locale - never stored, never sent, never parsed back into a number.
- **Round exactly once, at the point the value becomes final**, half-up. Rounding twice - once for display
  and again on persist - is how a total stops matching the sum of its own lines.
- **A total is derived, never entered twice.** If a line total and an order total can be written
  independently, they will disagree; compute one from the other and assert it.
- **Percentages apply to minor units and round immediately** to minor units. Carrying a fractional centavo
  into a second operation moves the error into the total.
- **Decide once where the remainder goes and write it here.** 33% of R$ 10,00 is 330 centavos, not
  333.333...; the leftover belongs to a named side of the transaction, and that choice gets a test.
- **Every monetary write is logged at a surviving level** with its identifier and the amount in minor units
  (§8.6). An amount is not personal data, and it is what makes a failed transaction reconcilable from a
  field capture.
- **Rounding goes through one shared helper, never inline.** `setScale`, `Math.round`,
  `kotlin.math.round` and `String.format("%.2f")` do not appear at a monetary call site: each one
  encodes a mode and a scale at the point of use, so two call sites drift apart silently and no test
  covers the disagreement. Route every monetary rounding through a single named utility in the shared
  module, with the mode as its documented default, and let that utility be the only place the
  primitive appears.

## 26. Root-Cause Bug Fixes (GLOBAL — MANDATORY)
