...
Build-tooling standards, shared verbatim across BipSale, Slotify, and Comiqueta (`CORE_RULES.md` §15). Sourced from Gradle/JetBrains/Google official best-practice guidance.
...
Every workflow that runs `./gradlew` validates the wrapper jar first, via `gradle/actions/wrapper-validation@v4` (or the dedicated `gradle-wrapper-validation.yml` workflow triggered on any change under `gradle/wrapper/**`). This checks the committed `gradle-wrapper.jar` against Gradle's known-good checksums before anything else runs — it is a supply-chain integrity gate, not a build step, so it must run before secrets are decoded or any other step executes.
