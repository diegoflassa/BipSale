# Agent Execution I/O — BipSale

How an AI agent must read, write and report in this repository. It owns three rules:

| § | Rule |
|---|---|
| **21** | [Text Encoding and Shell Output](#21-text-encoding-and-shell-output-global---mandatory) |
| **22** | [Chunked Writing of Long Artefacts](#22-chunked-writing-of-long-artefacts-global---mandatory) |
| **23** | [Token Economy Discipline](#23-token-economy-discipline-global---mandatory) |
...
## 21. Text Encoding and Shell Output (GLOBAL - MANDATORY)
...
### 21.1 Read and write bytes explicitly (MANDATORY)
...
### 21.2 The console is not evidence (MANDATORY)
...
### 21.3 Fingerprint before and after every edit (MANDATORY)
...
never infer it from a shell command's output (§21.2). Never let an editor or script
"helpfully" convert them; a whole-file EOL flip buries the real change in a diff nobody can review.

### 21.4 Never bulk-normalise encoding (MANDATORY)
...
## 22. Chunked Writing of Long Artefacts (GLOBAL - MANDATORY)
...
inverted, and §23 already forbids it: locate by search, edit by anchor.

---

## 23. Token Economy Discipline (GLOBAL - MANDATORY)
...
### 23.1 The priority order is fixed (MANDATORY)
...
§6, the fingerprint of §21.3, or reading a file you need to read in order not to
...
### 23.2 What is already in context is not read again (MANDATORY)
...
- **Re-read when — and only when — the file may have changed without you knowing:** after an edit
  ...
  §21.3 requires a fingerprint. There, re-reading is correct and the economy does not apply.
...
- **Economy is not guessing.** If what you have is a memory of the file rather than its contents,
  ...
  re-read — §23.1 decides.

### 23.3 Documentation sync: surgical, batched, at the end (MANDATORY)
...
### 23.4 Mechanical errors are the compiler's job, not yours (MANDATORY)
...
### 23.5 Keep the session's stable prefix intact (MANDATORY)
...
### 23.6 The report does not repeat what the operator already saw (MANDATORY)
