...
- **Hooks enforce what the rules only say.** Claude Code runs committed scripts from `.claude/settings.json`:
  ...
  `tools/hooks/check_agent_docs.py` (PostToolUse) reports frontmatter and link problems in an edited AI document. A third, `tools/hooks/session_start_index.py` (SessionStart), hands the agent the knowledge index.
