...
|---|---|---|---|
| [AI_BEHAVIOR.md](AI_BEHAVIOR.md) | Behavioural rules for every model — think before coding, simplicity, correctness first, KI sync, human-voice comments, deliverables in a box, task list on start, the 600-line file size limit. | Every task. | 8 KB |
| [CORE_RULES.md](CORE_RULES.md) | Standards, git safety, token economy, code style, KI discipline, regression tests, changelog, cross-project sync. **Holds the master index for the whole rule set.** | Every non-trivial change. Open the index, jump to the §N. | 26 KB |
...
| [DOC_GOVERNANCE.md](DOC_GOVERNANCE.md) | External reference documents and their versioning, where a newly agreed rule is written, and parity between the `conductor/`, `.agents/` and `.claude/` surfaces. | Using an outside document, agreeing a new rule, or editing `.agents/` or `.claude/`. | 18 KB |
...
| [PREVIEW_STANDARD.md](PREVIEW_STANDARD.md) | `@Preview` standard — device profiles, theme wrapper, localized mock data, 2+ states. | Adding or auditing any `@Preview`. | 7 KB |
...
| [AGENT_IO_RULES.md](AGENT_IO_RULES.md) | §21–§23 — text encoding and shell output, chunked writing of long artefacts, and the full token-economy spec behind CORE_RULES §3. | Editing any document with non-ASCII, writing a long file, or planning a long session. | 13 KB |
| [SECURITY_RULES.md](SECURITY_RULES.md) | §24 — what may never be committed, what a document carries instead, and the order of operations when a secret is already in git. | Touching `config-*.properties`, keystores, example scripts, or any captured evidence. | 4 KB |
| [CI.md](CI.md) | Manual pre-PR checklist. | Before opening a PR or merging. | 3 KB |
...
| Encoding, console evidence, chunked writes, token spec | [AGENT_IO_RULES](AGENT_IO_RULES.md) §21–§23 |
| Naming, no inline FQN, enum vs sealed, when an interface is justified | [CORE_RULES](CORE_RULES.md) §5 |
| File size — 200–400 line target, 600-line limit, decomposing a file past it | [AI_BEHAVIOR](AI_BEHAVIOR.md) §9 |
...
| `.agents/` rules, skills and workflows; their Claude Code pointers; Claude Code subagents; file names | [DOC_GOVERNANCE](DOC_GOVERNANCE.md) §18–§18.2 |
| Double-tap / single activation | [UI_RULES](UI_RULES.md) §19 |
| Secrets, keys, what never gets committed | [SECURITY_RULES](SECURITY_RULES.md) §24 |
| Which model runs a deferred task | [PLANNING_RULES](PLANNING_RULES.md) §25 · §25.6 |
| Money, rounding, minor units | [CORE_RULES](CORE_RULES.md) §20 |

## Also loaded by agents

`.agents/rules/*.md` and `.agents/skills/*/SKILL.md` are **pointers** into these files, never copies
([DOC_GOVERNANCE.md](DOC_GOVERNANCE.md) §18). When a rule here changes the moment at which it binds,
its rule's or skill's `description` changes in the same turn.
