...
|---|---|
| Has an observable outcome that commits - finalising a sale, recording a payment, moving stock, running an export or a backup restore | A guard held as a ViewModel property, released on the action's own outcome |
...
The exposure here is a duplicate sale, a duplicate stock movement, or a double-counted payment. A second
tap on *finalizar venda* lands easily on a counter device held by someone in a hurry, and the cost is not
noise - it is a transaction that has to be reconciled by hand against a customer who was charged once.
