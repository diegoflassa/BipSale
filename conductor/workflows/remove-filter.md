...
# /remove-filter <FILTER> — Log Filter Management

> Invoked as `/remove-filter <FILTER_NAME>`. Removes `Timber.d` / `Timber.v` calls tagged with a specific filter **while protecting application logic**. Also the runbook for adding a filter.
