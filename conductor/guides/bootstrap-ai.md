...
5. `conductor/rules/ARCHITECTURE.md` — **only when the task touches structure** (module graph, layers, MVI, Hilt, Room/Retrofit)
...
| `rules/ARCHITECTURE.md` | Module graph, layers, MVI, Hilt, Room/Retrofit, key files, build |
...
| Situation | Tier needed ([KI-10](../knowledge/KI-10-MODEL-SELECTION.md)) |
|---|---|
| Architecture decision / "impossible" bug | T1 - judgement |
| Room schema / migration, checkout money path | T2 or above (mandatory) |
| Feature implementation | T2 or above |
| Review / cross-check / pattern analysis | Any tier |
...
There is **no test coverage** in this repo yet ([`knowledge/TEST_COVERAGE.md`](../knowledge/TEST_COVERAGE.md)). Do not treat a green `./gradlew test` as evidence a change is safe.
