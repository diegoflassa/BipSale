...
class SessionStartTest(unittest.TestCase):
    def test_emits_the_knowledge_index(self):
        with tempfile.TemporaryDirectory() as root:
            os.makedirs(os.path.join(root, "conductor", "knowledge"))
            with open(os.path.join(root, "conductor", "knowledge", "INDEX.md"), "w", encoding="utf-8") as handle:
                handle.write("# KI Index\n")
            code, out = run("session_start_index.py", {"hook_event_name": "SessionStart"}, root)
            self.assertEqual(0, code)
            context = json.loads(out)["hookSpecificOutput"]["additionalContext"]
            self.assertTrue(context.startswith("[conductor/knowledge/INDEX.md]\n# KI Index"))
