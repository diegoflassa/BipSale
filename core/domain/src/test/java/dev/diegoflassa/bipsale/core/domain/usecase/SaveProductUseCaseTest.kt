...
class SaveProductUseCaseTest {
    ...
    private class FakeProductRepository : ProductRepository {
        ...
        override fun getAllProducts(): Flow<List<Product>> = flowOf(inserted.toList())
        override suspend fun getProductByCode(code: String): Product? =
            inserted.firstOrNull { it.code == code }
    ...
    fun `persists a valid product`() = runTest {
        ...
        assertThat(repository.inserted.first().price).isEqualTo(130.0)
    }
    ...
    fun `carries the image file name through to the stored product`() = runTest {
        ...
        assertThat(repository.inserted.first().imageFileName).isEqualTo("ct_a_ros_123.jpg")
    }
    ...
    fun `encodes code and price into the qr payload`() = runTest {
        ...
        assertThat(repository.inserted.first().qrCode)
