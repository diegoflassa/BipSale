...
Full spec (source of truth) - [PLANNING_RULES.md](../../../conductor/rules/PLANNING_RULES.md) §7 to §7.5 and §25.
...
- **Routing at the start of every task** (§25): **one recommended model** from the pool (§25.6), written with its
  ...
  guidance to get there - otherwise write `Fallback: none - <why>` (§25.4). Decision guide:
  [KI-10](../../../conductor/knowledge/KI-10-MODEL-SELECTION.md). **The plan is written by `Claude 5.0 Opus · Think ON · Effort Extra High`.** Put everything
...
5. **Write one plan file.** Do not write a sibling edition or a file per proposal. Route each task per §25: one
   recommended model from the pool with its fixed configuration, and a fallback from a different provider only
   when its code quality is the same (§25.4).
