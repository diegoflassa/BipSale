...
- **Every message carries exactly one scenario filter as its leading bracket tag** - `Timber.d("[BipSale][Sale] message")`.
  Parent is always `[BipSale]`; the child names the flow. A third segment names a step when needed.
- **The filter names what is diagnosed, never a ticket.** `[BipSale][BUG-123]` is forbidden.
...
- **Pick the level by asking - if this line is missing from a field capture, can the problem still be
  diagnosed?** If no, it may not be `Timber.d`. Failures, recoverable decisions and user-perceivable milestones
...
- **Every filter in the catalogue is protected** from `/clean` and `/remove-filter` sweeps.
